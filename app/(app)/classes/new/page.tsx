...
export default function NewClassPage() {
  ...
  async function onSubmit(data: ClassForm) {
    ...
    try {
      await api.createClass({ name: data.name, level: data.level, section: data.section || undefined })
      router.push('/classes')
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)' }}>
      <div style={{ padding: 'clamp(16px,4vw,28px) clamp(16px,4vw,32px) clamp(14px,3vw,24px)', background: 'var(--surface)', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 14 }}>
        <Link href="/classes" style={{ width: 34, height: 34, borderRadius: 9, background: 'var(--surface-2)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', textDecoration: 'none', flexShrink: 0 }}><ArrowLeft size={16} /></Link>
        <div>
          ...
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(18px,4vw,24px)', fontWeight: 700, color: 'var(--navy)', letterSpacing: '-0.02em' }}>Add New Class</h1>
        </div>
      </div>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden', marginBottom: 16 }}>
            ...
            <div style={{ padding: '22px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                ...
                <p style={{ fontFamily: 'system-ui', fontSize: 11, color: 'var(--text-muted)', marginTop: 5 }}>Auto-generated from Level + Section.</p>
              </div>
            </div>
          </div>
