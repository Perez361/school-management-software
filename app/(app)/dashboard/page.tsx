...
export default function DashboardPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)' }}>
              ...
              {recentPayments.length === 0 ? (
                ...
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderBottom: i < recentPayments.length - 1 ? '1px solid var(--border-soft)' : 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div>
                      <div style={{ fontFamily: 'system-ui', fontSize: 13, fontWeight: 600, color: 'var(--navy)' }}>{p.student?.name}</div>
                      <div style={{ fontFamily: 'system-ui', fontSize: 11, color: 'var(--text-muted)' }}>{p.student?.class?.name} · {p.feeType} · {p.term}</div>
                    </div>
                  </div>
      ...
      <style>{`@keyframes pulse { 0% { transform: scale(1); opacity: 0.8; } 50% { transform: scale(1.5); opacity: 0.4; } 100% { transform: scale(1); opacity: 0.8; } }`}</style>
    </div>
  )
}
