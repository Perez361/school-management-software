...
export default function NewPaymentPage() {
  ...
  const { register, handleSubmit, watch } = useForm<PaymentForm>({
    defaultValues: { term: 'Term 1', feeType: 'Tuition', amount: '1500', paid: '0' }
  })
  ...
  useEffect(() => {
    if (!classId) return
    api.getStudents({ classId: parseInt(classId) }).then(setStudents)
  }, [classId])
  ...
  async function onSubmit(data: PaymentForm) {
    ...
    try {
      ...
      router.push('/billing')
    } catch (e: any) { setError(e.message || e) } finally { setLoading(false) }
  }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--cream)' }}>
      <div style={{ padding: 'clamp(16px,4vw,28px) clamp(16px,4vw,32px) clamp(14px,3vw,24px)', background: 'var(--surface)', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <Link href="/billing" style={{ width: 34, height: 34, borderRadius: 9, background: 'var(--surface-2)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', textDecoration: 'none', flexShrink: 0 }}><ArrowLeft size={16} /></Link>
          <div>
            ...
            <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(18px,4vw,24px)', fontWeight: 700, color: 'var(--navy)', letterSpacing: '-0.02em' }}>Record Payment</h1>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(22,163,74,0.07)', border: '1px solid rgba(22,163,74,0.15)', borderRadius: 9, padding: '8px 14px', flexShrink: 0 }}>
      ...
      <div style={{ padding: 'clamp(12px,3vw,28px) clamp(16px,4vw,32px)', maxWidth: 680 }}>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden', marginBottom: 16 }}>
            ...
            <div style={{ padding: '22px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 14 }}>
              <div>
                {lbl('Filter by Class')}
          ...
          <div style={{ borderRadius: 12, padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, background: isFullyPaid ? 'rgba(22,163,74,0.07)' : 'rgba(217,119,6,0.07)', border: `1px solid ${isFullyPaid ? 'rgba(22,163,74,0.2)' : 'rgba(217,119,6,0.2)'}` }}>
            <div>
              <div style={{ fontFamily: 'system-ui', fontSize: 11, fontWeight: 700, color: isFullyPaid ? '#15803d' : '#b45309', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>Outstanding Balance</div>
              <div style={{ fontFamily: 'system-ui', fontSize: 11, color: isFullyPaid ? '#15803d' : '#b45309' }}>{isFullyPaid ? 'Fully paid — no balance remaining' : `GHS ${paid.toFixed(2)} of GHS ${amount.toFixed(2)} paid`}</div>
            </div>
            <div style={{ fontFamily: 'Georgia, serif', fontSize: 24, fontWeight: 700, color: isFullyPaid ? '#15803d' : '#b45309' }}>GHS {balance.toFixed(2)}</div>
          </div>
