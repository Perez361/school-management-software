...
export default async function ReportsPage() {
  ...
  const reportTypes = [
    {
      type: 'report-card' as const,
      title: 'Terminal Report Cards',
      description: 'Generate individual PDF report cards with grades, scores, and class position for each student',
      icon: FileText,
      color: '#2563eb',
    ...
    {
      type: 'class-list' as const,
      title: 'Class Registers',
      description: 'Print full class lists with student IDs, names, gender, and parent contact information',
      icon: Users,
      color: '#15803d',
    ...
    {
      type: 'fee-invoice' as const,
      title: 'Fee Invoices',
      description: 'Generate billing invoices showing amounts due, paid, and outstanding balances per student',
      icon: Receipt,
      color: '#b45309',
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontFamily: 'system-ui', fontSize: 12, color: 'var(--text-secondary)', marginTop: 5 }}>
            Generate and download PDF documents for printing or sharing
          </p>
        </div>
        <div style={{
          ...
          <span style={{ fontFamily: 'system-ui', fontSize: 12, color: 'var(--text-secondary)', fontWeight: 500 }}>
            {classes.length} classes · {settings?.schoolName || 'School'}
          </span>
        </div>
      </div>
