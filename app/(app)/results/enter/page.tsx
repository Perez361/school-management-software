...
import { ArrowLeft, Save, CheckCircle, BookOpen, Users, ClipboardList } from 'lucide-react'
import { api, Student, Subject, Class } from '@/lib/api'
...
export default function EnterResultsPage() {
  ...
  // ca is auto-loaded (read-only); exam is manually entered
  const [caScores,  setCAScores]  = useState<Record<number, number | null>>({})
  const [scores,    setScores]    = useState<Record<number, { exam: string }>>({})
  ...
  useEffect(() => {
    ...
    api.getStudents({ classId: parseInt(classId) }).then(data => {
      setStudents(data)
      const init: Record<number, { exam: string }> = {}
      data.forEach(s => { init[s.id] = { exam: '' } })
      setScores(init)
    })
  }, [classId])

  // Load CA scores + existing exam results when subject/term/year changes
  const loadData = useCallback(() => {
    ...
    Promise.all([
      ...
      // Map CA computed values per student
      const caMap: Record<number, number | null> = {}
      caRows.forEach(ca => {
        caMap[ca.studentId] = ca.computedCA ?? null
      })
      setCAScores(caMap)

      // Pre-fill exam scores from existing results
      setScores(prev => {
        const updated: Record<number, { exam: string }> = {}
        students.forEach(s => { updated[s.id] = prev[s.id] || { exam: '' } })
        resultRows
          .filter(r => r.subjectId === sid)
          .forEach(r => { updated[r.studentId] = { exam: String(r.exam) } })
        return updated
      })
    })
  ...
  async function saveAll() {
    ...
    try {
      const entries = Object.entries(scores).filter(([, v]) => v.exam !== '')
      await Promise.all(entries.map(([sid, { exam }]) => {
  ...
  const filledCount = Object.values(scores).filter(v => v.exam !== '').length
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)' }}>
      ...
      <div style={{ padding: 'clamp(16px,4vw,28px) clamp(16px,4vw,32px) clamp(14px,3vw,24px)', background: 'var(--surface)', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 14 }}>
        <Link href="/results" style={{ width: 34, height: 34, borderRadius: 9, background: 'var(--surface-2)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', textDecoration: 'none', flexShrink: 0 }}>
          <ArrowLeft size={16} />
        </Link>
        <div>
          ...
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(20px,4vw,26px)', fontWeight: 700, color: 'var(--navy)', letterSpacing: '-0.02em' }}>Enter Exam Results</h1>
          <p style={{ fontFamily: 'system-ui', fontSize: 12, color: 'var(--text-secondary)', marginTop: 3 }}>
            CA is auto-loaded from Cumulative Assessments — enter only the end-of-term exam score
          </p>
        </div>
      </div>
        ...
        {classId && subjectId && students.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', background: caLoadedCount > 0 ? 'rgba(201,168,76,0.07)' : 'rgba(217,119,6,0.06)', border: `1px solid ${caLoadedCount > 0 ? 'rgba(201,168,76,0.25)' : 'rgba(217,119,6,0.2)'}`, borderRadius: 10 }}>
            ...
            <span style={{ fontFamily: 'system-ui', fontSize: 12, color: caLoadedCount > 0 ? 'var(--navy)' : '#92400e', fontWeight: 500 }}>
              {caLoadedCount > 0
                ? <><strong>{caLoadedCount}</strong> of {students.length} students have CA scores loaded from Cumulative Assessments</>
                : <>No CA scores found for this subject/term/year — <Link href="/ca-scores" style={{ color: 'var(--navy)', fontWeight: 700 }}>enter CA scores first</Link></>
              }
            </span>
          </div>
        )}
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontFamily: 'system-ui', fontSize: 11, color: 'var(--text-muted)' }}>{filledCount} exam scores entered</div>
                </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                >
                  <Save size={14} />{saving ? 'Saving…' : `Save ${filledCount > 0 ? filledCount : ''} Result${filledCount !== 1 ? 's' : ''}`}
                </button>
              </div>
            </div>
            ...
            <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' } as React.CSSProperties}>
              <div style={{ minWidth: 580 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '36px 1fr 110px 100px 120px 60px', gap: 10, padding: '10px 20px', background: 'var(--gold-pale)', borderBottom: '1px solid var(--border)' }}>
                  {['#', 'Student Name', 'Student ID', 'CA /30', 'Exam /70', 'Total'].map(h => (
                ...
                {students.map((s, i) => {
                  const sc    = scores[s.id] || { exam: '' }
                  const ca    = caScores[s.id] ?? null
                  const exam  = parseFloat(sc.exam)
                  const total = ca !== null && !isNaN(exam) ? Math.round((ca + exam) * 100) / 100 : null
                  ...
                  return (
                    <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '36px 1fr 110px 100px 120px 60px', gap: 10, padding: '10px 20px', alignItems: 'center', borderBottom: i < students.length - 1 ? '1px solid var(--border-soft)' : 'none' }}>
                      ...
                      <input
                        ...
                        value={sc.exam}
                        onChange={e => setScores(prev => ({ ...prev, [s.id]: { exam: e.target.value } }))}
                        placeholder="0 – 70"
                        style={{ width: '100%', padding: '7px 10px', textAlign: 'center', background: 'var(--surface-2)', border: `1.5px solid ${sc.exam !== '' ? 'var(--navy)' : 'var(--border)'}`, borderRadius: 8, fontFamily: 'system-ui', fontSize: 13, color: 'var(--text-primary)', outline: 'none' }}
                      />

                      {/* Total */}
                      <div style={{ textAlign: 'center' }}>
                        {total !== null
                          ? <span style={{ display: 'inline-block', fontFamily: 'Georgia, serif', fontSize: 14, fontWeight: 700, color: totalColor, background: totalBg, padding: '3px 8px', borderRadius: 7, minWidth: 40, textAlign: 'center' }}>{total}</span>
