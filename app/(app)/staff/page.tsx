            ...
            {Object.entries(roleCounts).map(([role, count]) => {
              ...
              return (
                <span key={role} style={{
                  ...
                  color: m.color, background: m.bg, padding: '2px 9px', borderRadius: 20,
                  display: 'inline-flex', alignItems: 'center', gap: 5,
                }}>
                  <span style={{ width: 5, height: 5, borderRadius: '50%', background: m.dot, display: 'inline-block' }} />
                  {count} {role}{count > 1 && role !== 'Staff' ? 's' : ''}
                </span>
              )
            })}
          </div>
        </div>
        <Link href="/staff/new" style={{
                ...
                {staff.map((s, i) => {
                  ...
                  return (
                    <tr key={s.id} style={{ borderBottom: i < staff.length - 1 ? '1px solid var(--border-soft)' : 'none' }}>
                      <td style={{ padding: '13px 18px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                          <div style={{
                            ...
                            fontFamily: 'system-ui', fontSize: 11, fontWeight: 700, color: m.color, flexShrink: 0,
                          }}>
                      ...
                      <td style={{ padding: '13px 18px' }}>
                        <span style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--text-muted)', background: 'var(--surface-2)', padding: '3px 8px', borderRadius: 5 }}>
                          {s.staffId}
                        </span>
                      </td>
                      <td style={{ padding: '13px 18px' }}>
                      ...
                      <td style={{ padding: '13px 18px', fontSize: 13, color: 'var(--text-secondary)' }}>
                        {s.subject || <span style={{ color: 'var(--text-muted)' }}>—</span>}
                      </td>
                      <td style={{ padding: '13px 18px' }}>
                        {s.class ? (
                          <span style={{
                      ...
                      <td style={{ padding: '13px 18px', fontSize: 13, color: 'var(--text-secondary)' }}>
                        {s.phone || <span style={{ color: 'var(--text-muted)' }}>—</span>}
                      </td>
                      <td style={{ padding: '13px 18px' }}>
                        <Link href={`/staff/${s.id}/edit`} style={{
                ...
                {staff.length === 0 && (
                  <tr>
                    <td colSpan={7} style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-muted)' }}>
                      ...
                      }}><Plus size={13} /> Add First Staff Member</Link>
                    </td>
                  </tr>
                )}
              </tbody>
